= Test-Suite: Run RGR With Phase Timeout
...
== Test-Case: Green claude killed and install passes, refactor unaffected
...
== Test-Case: Green killed, install fails, resumed claude fixes it
...
== Test-Case: Green timeouts exhaust max attempts
...
== Test-Case: Refactor killed, install fails, resumed claude fixes it
...
== Test-Case: Refactor timeouts exhaust max attempts
...
== Test-Case: Claude process exits but stdout stays open, reader-side timeout fires
