= Test-Suite: Run RGR With Branch Verification
...
== Test-Case: GitBranch parameter is required
...
== Test-Case: GitBranch mismatch aborts the run
...
== Test-Case: Detached HEAD aborts the run
...
== Test-Case: Matching branch records git_branch on every metrics row
