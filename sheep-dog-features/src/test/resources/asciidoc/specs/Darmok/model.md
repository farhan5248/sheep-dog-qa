...
Per-scenario reset (issue 337): the `ClaudeRunner` instances are constructed once in `init()` and reused across every scenario in the run. Without intervention the green and refactor runners cache the first scenario's UUID and reuse it for every subsequent scenario, which the real Claude CLI rejects with `Error: Session ID <uuid> is already in use.` on the second scenario's initial call. `processScenario` calls `setSessionId(null)` on both runners at the top of each iteration so the next initial call generates a fresh UUID. Living-documentation Test-Case in `Scenario Loop Multiple Scenarios.asciidoc` asserts two distinct UUIDs across two scenarios; integration tests pin the runtime behavior.

Non-retryable failure visibility (issue 337): on the `NonRetryable` exit, claude's last stdout line is mirrored to the mojo log at ERROR level (`Claude failed (exit N): <last-line>`). Previously only the runner log carried the failure reason — the mojo log just showed `<phase>: Completed` and the user had to dig into the runner log to find what claude actually said.
