= Test-Suite: Directory Allowlist
...
== Test-Setup: A failing scenario that reaches the allowlist sub-step
...
== Test-Case: Green allowlist passes on the first attempt
...
== Test-Case: Refactor allowlist passes on the first attempt
...
== Test-Case: Green allowlist fails once then succeeds after revert and resume
...
== Test-Case: Green allowlist fails for every attempt
...
== Test-Case: Refactor allowlist fails once then succeeds after revert and resume
...
== Test-Case: Refactor allowlist fails for every attempt
...
== Test-Case: Green allowlist passes when an extra path is added via allowlistAdditionalPaths
...
== Test-Case: Green allowlist fails when allowlistBasePaths narrows below default
...
== Test-Case: Green allowlist retry brackets the resume call with Running and Completed lines
...
== Test-Case: Refactor allowlist retry brackets the resume call with Running and Completed lines
...
== Test-Case: Active scenarios-list file is allowlisted automatically
...
== Test-Case: Non-matching scenarios-list-shaped file is treated as a violation
...
== Test-Case: Path under src/test/resources is allowlisted by default
