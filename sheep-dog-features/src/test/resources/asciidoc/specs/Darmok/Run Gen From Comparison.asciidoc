= Test-Suite: Run Gen From Comparison

@darmok-maven-plugin

The `gen-from-comparison` goal invokes the `/rgr-gen-from-comparison` Claude skill before each scenario-processing iteration.
When the comparison skill succeeds, the subsequent RGR cycle runs normally; when it fails, the mojo aborts before any scenario is handled (failure path deferred to #258).

== Test-Setup: A pending scenario with implementation absent
...
== Test-Case: Comparison skill runs, then the normal RGR cycle completes

=== When: The maven plugin gen-from-comparison goal is executed with
|===
| ModelComparison | ModelGreen | ModelRefactor | Pipeline
| sonnet          | sonnet     | sonnet        | forward
|===
